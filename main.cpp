...
#include "./tables/gltransactions.h"
...
int main(int argc, char *argv[])
{
    ...
    app.get_middleware<JWTMiddleWare>().setMessage("Nobleledger Accounting");

    app.loglevel(crow::LogLevel::Info);

    CROW_ROUTE(app, "/book_journal/<int>")
    ([](int transaction_id)
     {  GLJournalTransactions transactions;
        const int r = transactions.bookTransaction(transaction_id);
        const string s = to_string(transaction_id);
        return "<h1>Application Server</h1><h2>Booked transaction " + s + "<h2>"; });
